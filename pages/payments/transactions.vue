            ...
            <CardContent>
                ...
                <div class="flex items-center justify-between mt-4">
                    <div class="text-sm text-muted-foreground">
                        Showing {{ transactions.length }} of {{ transactionsQuery.data.value?.count || 0 }} transactions
                    </div>
                        ...
                        <div v-if="transaction.failedPayments && transaction.failedPayments.length > 0"
                            ...
                            <div class="flex items-start gap-2">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-xs text-red-700">{{ transaction.failedPayments[0].userFriendlyMessage
                                    }}</p>
                                    <Button v-if="transaction.failedPayments[0].canRetry" variant="outline" size="sm"
                                        class="mt-2" @click.stop="retryPayment(transaction.failedPayments[0].id)">
                                        Retry Payment
                                    </Button>
                                </div>
                            </div>
...
const filterDefinitions = [
    ...
    { field: 'date_to', type: 'date' as const }
]
...
const transactionsQuery = useTransactions(currentQuery)
const transactions = computed(() => transactionsQuery.data.value?.results || [])
const isLoading = computed(() => transactionsQuery.isLoading.value)
