    ...
    <Card>
        <div class="flex items-center justify-between px-4 py-3 border-b">
            <div class="relative">
                ...
                <Input type="text" placeholder="Search tickets..." class="rounded-full w-64 pl-10"
        ...
        <div class="p-4">
            ...
            <div v-else-if="tickets.length === 0" class="text-center py-12">
                <EmptyState title="No tickets found" description="Create your first ticket to get started"
                    action-label="Create Ticket" @action="$emit('create')" />
            </div>

            <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <TicketCard v-for="ticket in tickets" :key="ticket.id" :ticket="ticket" :can-assign="canAssign"
                    :can-escalate="canEscalate" @click="$emit('view', ticket)" @view="$emit('view', ticket)"
                    @assign="$emit('assign', ticket)" @escalate="$emit('escalate', ticket)" />
            </div>
        </div>
    </Card>
</template>
