...
export function useMaintenance(){
  ...
  async function add(item: MaintenanceItem){
    const res = await $fetch<{ item: MaintenanceItem }>(`/api/property/maintenance`, { method:'POST', body: item })
    items.value.push(res.item)
  }
  async function update(id: string, patch: Partial<MaintenanceItem>){
    const res = await $fetch<{ item: MaintenanceItem }>(`/api/property/maintenance/${id}`, { method:'PUT', body: patch })
    const i = items.value.findIndex(x=>x.id===id); if (i>=0) items.value[i] = res.item
  }
  function exportCsv(){
